...
class Ficha(db.Model):
    ...
    def calcula_coeficientes(self):
        ...
        conceitos_validos = ['A', 'B', 'C', 'D', 'E']
        situacaos_validas = ['aprovado', 'disc.equiv', 'apr.s.nota']
        for disciplina in self.disciplinas:
            if disciplina.conceito in conceitos_validos or disciplina.situacao.lower() in situacaos_validas:
            ...
            if disciplina.conceito == 'A':
                ...
                peso_do_conceito = 0
            if (disciplina.conceito != 'E') and (disciplina.conceito != '-') and (disciplina.conceito != 'I') and (
        ...
        self.coeficiente_cp = (self.creditos_de_obrigatorias + self.creditos_de_limitadas + self.creditos_de_livres) / (
                self.meta_das_obrigatorias + self.meta_das_limitadas + self.meta_das_livres)
        print("coeficiente_cp = %s" % (self.coeficiente_cp))
        ...
        for disciplina in self.disciplinas:
            ...
            for disciplina_aux in self.disciplinas:
                if (disciplina.nome.upper() == disciplina_aux.nome.upper()):
                    if (disciplina_aux.conceito < disciplina.conceito) and (disciplina_aux.conceito != 'O') and (
                            disciplina_aux.conceito != 'I'):
        ...
        for disciplina_ca in disciplinas_ca:
            if disciplina_ca.conceito == 'A':
                ...
                peso_do_conceito = 0
            if (disciplina_ca.conceito != 'E') and (disciplina_ca.conceito != '-') and (
                    disciplina_ca.conceito != 'I') and (disciplina_ca.categoria != '-'):
