from flask import Flask, json, jsonify, request, render_template, redirect, url_for, session, flash, Markup, abort, \
    send_file
...
def upload_json():
    ...
    ficha = Ficha()
    curso = request.form['select-curso']
    if curso == "bct":
        ficha.meta_das_obrigatorias = 90
        ficha.meta_das_limitadas = 57
        ficha.meta_das_livres = 43
    elif curso == "bcc":
        ficha.meta_das_obrigatorias = 214
        ficha.meta_das_limitadas = 30
        ficha.meta_das_livres = 12
    elif curso == "ei":
        ficha.meta_das_obrigatorias = 245
        ficha.meta_das_limitadas = 28
        ficha.meta_das_livres = 27
...
def ficha_atualizar_disciplina():
    ...
    for disciplina in ficha.disciplinas:
        if disciplina.codigo == data['codigo'] and disciplina.ano == data['ano'] and disciplina.periodo == data[
            'periodo']:
